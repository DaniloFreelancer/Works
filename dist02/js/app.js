(() => {
    ...
    da.init();
    const exploreItems = document.querySelector(".explore__items");
    if (exploreItems) loadExploreItems();
    ...
    function initExplore(data) {
        for (let index = 0; index < 3; index++) {
            ...
            console.log(item);
            buildExploreItem(item);
        }
    }
    function buildExploreItem(item) {
        let exploreItemTemplate = ``;
        exploreItemTemplate += `<article class="explore__item item-explore">`;
        ...
        if (item.tags) {
            ...
            for (const tag in item.tags) exploreItemTemplate += `<a href="${item.tags[tag]}" class="item-explore__tag">${tag}</a>`;
            exploreItemTemplate += `</div>`;
        }
        exploreItemTemplate += `</article>`;
        exploreItems.insertAdjacentHTML("beforeend", exploreItemTemplate);
    }
    window["FLS"] = true;
    addLoadedClass();
    menuInit();
