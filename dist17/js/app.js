    ...
    (() => {
        ...
        function formFieldsInit(options = {
            ...
            if (options.viewPass) document.addEventListener("click", (function(e) {
                ...
                if (targetElement) {
                    let inputElement = targetElement.parentElement.querySelector("input");
                    if (inputElement) {
        ...
        function formSubmit() {
            ...
            function formSent(form, responseResult = ``) {
                ...
                formValidate.formClean(form);
                formLogging(`Form submitted succesfully!`);
            }
        ...
        if (datepickerElements.length) datepickerElements.forEach((element => {
            ...
            if (!flsModules.datepickers) flsModules.datepickers = [];
        ...
        function initSliders() {
            if (document.querySelector(".explore__slider")) new swiper_core_Swiper(".explore__slider", {
                ...
                on: {
                    slideChange: function() {
                        ...
                        if (currentIndex > 0) prevButton.classList.add("swiper-button-disabled"); else prevButton.classList.remove("swiper-button-disabled");
                        if (currentIndex === 0) nextButton.classList.add("swiper-button-disabled");
                    }
                }
            });
                ...
                on: {
                    slideChange: function() {
                        ...
                        prevButton.forEach((button => {
                            if (currentIndex > 0) button.classList.add("swiper-button-disabled"); else button.classList.remove("swiper-button-disabled");
                        }));
                        nextButton.forEach((button => {
                            if (currentIndex === 0) button.classList.add("swiper-button-disabled");
                        }));
                    }
                }
            });
            if (document.querySelector(".about__slider")) {
                const mainSwiper = new swiper_core_Swiper(".swiper1", {
                    modules: [ Navigation ],
                    observer: true,
                    observeParents: true,
                    loop: true,
                ...
                new swiper_core_Swiper(".mySwiper", {
...
***************************************************************************** */
        ...
        document.querySelectorAll(".menu__link._active").forEach((link => {
            link.classList.remove("_active");
        }));
        document.querySelectorAll(".menu__link").forEach((link => {
            if (link.getAttribute("href") === currentPage) link.classList.add("_active");
        }));
        document.addEventListener("DOMContentLoaded", (() => {
            const wrapper = document.getElementById("wrap");
            const header = document.querySelector("header");
            const footer = document.querySelector("footer");
            if (wrapper) header.classList.add("header--color");
            if (currentPage === "contact.html") footer.classList.add("footer--padding-top");
        }));
        const allowedPages = [ "about.html", "packages.html", "tour.html", "contact.html", "purchase.html" ];
        const loadedPage = window.location.pathname;
        if (allowedPages.some((page => loadedPage.includes(page)))) window.addEventListener("load", (() => {
            setTimeout((() => {
                document.documentElement.classList.add("loaded");
            }), 300);
        }));
        if (loadedPage.includes("home.html") || allowedPages.some((page => loadedPage.includes(page)))) window.addEventListener("load", (() => {
            setTimeout((() => {
                alert("Translation of the page is not available at the moment!");
            }), 300);
        }));
        const customSelect = document.querySelector('.booking-forms__item [data-id="1"]');
        const dateElement = document.getElementById("date-content");
        const selectContent = document.querySelector(' .booking-forms__item [data-id="1"] .select__content');
        const customSelect_2 = document.querySelector('.booking-forms__item [data-id="2"]');
        const dateElement_2 = document.getElementById("time-content");
        const selectContent_2 = document.querySelector(' .booking-forms__item [data-id="2"] .select__content');
        function changeValues(customSelect, idElement, selectContent) {
            if (customSelect && idElement) {
                idElement.textContent = selectContent.textContent;
                customSelect.addEventListener("click", (event => {
                    const option = event.target.closest(".select__option");
                    if (option) {
                        const selectedText = option.textContent;
                        idElement.textContent = selectedText;
                    }
                }));
            }
        }
        changeValues(customSelect, dateElement, selectContent);
        changeValues(customSelect_2, dateElement_2, selectContent_2);
        const ticketCounters = document.querySelectorAll(".ticket-booking");
        ticketCounters.forEach((ticketBlock => {
            const counterElement = ticketBlock.querySelector("#counter");
            const increaseButton = ticketBlock.querySelector(".ticket-booking__increase");
            const decreaseButton = ticketBlock.querySelector(".ticket-booking__decrease");
            const description = ticketBlock.querySelector("#description").textContent;
            const pricePerUnit = parseFloat(ticketBlock.querySelector("[data-price]").getAttribute("data-price"));
            const contentContainer = document.querySelector(".overview-body__content");
            const totalElement = document.querySelector("#total");
            function updateOverview() {
                const currentCount = parseInt(counterElement.textContent, 10);
                let overviewItem = Array.from(contentContainer.querySelectorAll(".overview-body__item-content")).find((item => item.querySelector(".overview-body__description").textContent === description));
                if (currentCount === 0 && overviewItem) overviewItem.remove(); else if (currentCount > 0) {
                    const totalPrice = currentCount * pricePerUnit;
                    if (!overviewItem) {
                        overviewItem = document.createElement("div");
                        overviewItem.classList.add("overview-body__item-content");
                        overviewItem.innerHTML = `\n          <div class="overview-body__count">${currentCount}</div>\n          <div class="overview-body__description">${description}</div>\n          <span class="overview-body__price">€${totalPrice.toFixed(2)}</span>\n        `;
                        contentContainer.appendChild(overviewItem);
                    } else {
                        overviewItem.querySelector(".overview-body__count").textContent = currentCount;
                        overviewItem.querySelector(".overview-body__price").textContent = `€${totalPrice.toFixed(2)}`;
                    }
                }
            }
            function updateTotal(totalElement) {
                let total = 0;
                ticketCounters.forEach((ticket => {
                    const price = parseFloat(ticket.querySelector("[data-price]").getAttribute("data-price"));
                    const count = parseInt(ticket.querySelector("#counter").textContent, 10);
                    total += price * count;
                }));
            ...
            increaseButton.addEventListener("click", (() => {
                ...
                decreaseButton.classList.remove("_disable");
                updateOverview();
                updateTotal(totalElement);
            }));
            decreaseButton.addEventListener("click", (() => {
                ...
                if (currentCount > 0) {
                    currentCount--;
                    counterElement.textContent = currentCount;
                    if (currentCount === 0) decreaseButton.classList.add("_disable");
                    updateOverview();
                    updateTotal(totalElement);
                }
            }));
        ...
        function toggleCheckIcon(input, isValid) {
            const checkIcon = input.parentElement.querySelector(".check-icon");
            if (isValid) checkIcon.classList.remove("hidden"); else checkIcon.classList.add("hidden");
        }
        document.addEventListener("DOMContentLoaded", (() => {
            ...
            if (cardNumberInput) cardNumberInput.addEventListener("input", (e => {
                ...
                toggleCheckIcon(e.target, validateCardNumber(e.target.value));
            }));
            if (expirationDateInput) expirationDateInput.addEventListener("input", (e => {
                const value = e.target.value.replace(/\D/g, "");
                e.target.value = value.length >= 3 ? value.slice(0, 2) + "/" + value.slice(2, 4) : value;
                toggleCheckIcon(e.target, validateExpirationDate(e.target.value));
            }));
            if (securityCodeInput) securityCodeInput.addEventListener("input", (e => {
                toggleCheckIcon(e.target, validateSecurityCode(e.target.value));
            }));
        }));
        document.querySelectorAll(".options__input").forEach((radio => {
            radio.addEventListener("change", (() => {
                const paymentItems = document.querySelectorAll(".form-payment__item");
                paymentItems.forEach((item => {
                    item.classList.remove("form-payment__item--active");
                }));
                const parentItem = radio.closest(".form-payment__item");
                if (parentItem) parentItem.classList.add("form-payment__item--active");
                if (parentItem && parentItem.id === "paypal") setTimeout((() => {
                    window.open("https://www.paypal.com/ua/signin", "_blank");
                }), 1e3);
            }));
        }));
        const container = document.querySelector(".purchase__container");
        const nextBtns = document.querySelectorAll(".next-btn");
        const progress = document.getElementById("progress");
        const formSteps = document.querySelectorAll(".forms-purchase-step");
        ...
        function validateForm(form) {
            ...
            requiredFields.forEach((field => {
                ...
                } else if (field.dataset.required === "email") {
        ...
        nextBtns.forEach((button => {
            button.addEventListener("click", (() => {
                const activeForm = document.querySelector(".forms-purchase-step-active form");
                if (activeForm && !validateForm(activeForm)) return;
                formStepsNum++;
                if (formStepsNum < formSteps.length && formStepsNum < progressSteps.length) {
                    updateFormSteps(formSteps, formStepsNum);
                    updateProgressbar(formStepsNum, progressSteps, progress);
                } else container.innerHTML = ` \n\t\t\t\t\t<div class="purchase__complete complete-purchase">\n\t\t\t\t\t\t<svg class="complete-purchase__icon" width="100" height="100" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">\n\t\t\t\t\t\t\t<path fill-rule="evenodd" clip-rule="evenodd"\n\t\t\t\t\t\t\t\td="M50.0003 4.16669C24.6878 4.16669 4.16699 24.6875 4.16699 50C4.16699 75.3125 24.6878 95.8334 50.0003 95.8334C75.3128 95.8334 95.8337 75.3125 95.8337 50C95.8337 24.6875 75.3128 4.16669 50.0003 4.16669ZM69.867 42.25C70.2328 41.8319 70.5113 41.3448 70.6861 40.8174C70.8609 40.29 70.9284 39.733 70.8848 39.1791C70.8411 38.6252 70.6871 38.0857 70.4319 37.5922C70.1766 37.0987 69.8253 36.6613 69.3984 36.3056C68.9716 35.9499 68.478 35.6832 67.9466 35.5211C67.4151 35.359 66.8567 35.3049 66.304 35.3618C65.7513 35.4187 65.2156 35.5856 64.7284 35.8527C64.2412 36.1197 63.8123 36.4815 63.467 36.9167L45.5503 58.4125L36.2795 49.1375C35.4936 48.3785 34.4411 47.9585 33.3487 47.968C32.2562 47.9775 31.2111 48.4157 30.4386 49.1883C29.666 49.9608 29.2278 51.0059 29.2183 52.0983C29.2089 53.1908 29.6288 54.2433 30.3878 55.0292L42.8878 67.5292C43.2972 67.9383 43.7874 68.2577 44.3271 68.4669C44.8667 68.6762 45.4441 68.7707 46.0223 68.7444C46.6005 68.7181 47.1669 68.5717 47.6854 68.3144C48.2038 68.0571 48.663 67.6946 49.0337 67.25L69.867 42.25Z"\n\t\t\t\t\t\t\t\tfill="#FA8B02" />\n\t\t\t\t\t\t</svg>\n\t\t\t\t\t\t<h3 class="complete-purchase__title">Your Order is complete!</h3>\n\t\t\t\t\t\t<div class="complete-purchase__text">\n\t\t\t\t\t\t\t<p>\n\t\t\t\t\t\t\t\tYou will be receiving a confirmation email with order details.\n\t\t\t\t\t\t\t</p>\n\t\t\t\t\t\t</div>\n\t\t\t\t\t\t<a href="home.html" target="_blank" class="complete-purchase__link-button">Go to the Home Page</a>\n\t\t\t\t\t</div>\n\t\t\t`;
            }));
        }));
        function updateFormSteps(formSteps, num) {
            formSteps.forEach((formStep => {
                if (formStep.classList.contains("forms-purchase-step-active")) formStep.classList.remove("forms-purchase-step-active");
            }));
            formSteps[num].classList.add("forms-purchase-step-active");
        }
        function updateProgressbar(num, progressSteps, progress) {
            progressSteps.forEach(((progressStep, index) => {
                if (index < num + 1) progressStep.classList.add("progress-bar__step--active"); else progressStep.classList.remove("progress-bar__step--active");
                const progressStepActive = document.querySelectorAll(".progress-bar__step--active");
                progress.style.width = (progressStepActive.length - 1) / (progressSteps.length - 1) * 100 + "%";
            }));
        }
        document.addEventListener("DOMContentLoaded", (() => {
            const signUpButton = document.querySelector(".form-popup__signUp-button");
            if (signUpButton) signUpButton.addEventListener("click", (function(e) {
                e.preventDefault();
            ...
            const checkbox = document.querySelector(".form-popup__checkbox");
            const buttons = document.querySelector(".form-popup__buttons");
            if (checkbox && buttons) checkbox.addEventListener("change", (() => {
                if (checkbox.checked) buttons.classList.remove("form-popup__buttons--disable"); else buttons.classList.add("form-popup__buttons--disable");
            }));
            ...
            if (signInButton) signInButton.addEventListener("click", (e => {
                ...
                if (savedUser && savedUser.email === email && savedUser.password === password) alert("Login successful!"); else alert("Invalid email or password.");
            })); else console.error("Sign-in button not found in the DOM.");
            const resetPassword = document.querySelector(".reset-password");
            if (resetPassword) resetPassword.addEventListener("click", (() => {
                e.preventDefault();
                ...
                alert(`New password has been sent to your email: ${savedUser.email}\nNew Password: ${newPassword}`);
            }));
