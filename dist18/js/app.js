(() => {
    ...
    function initSliders() {
        if (document.querySelector(".swiper")) {
            new swiper_core_Swiper(".item-features__slider", {
            ...
            new swiper_core_Swiper(".cases__slider", {
            ...
            const unreverse = new swiper_core_Swiper(".unreverse", {
                modules: [ Navigation, Autoplay ],
                observer: true,
                observeParents: true,
                slidesPerView: 4,
            ...
            const reverse = new swiper_core_Swiper(".reverse", {
                modules: [ Navigation, Autoplay ],
                observer: true,
                observeParents: true,
                slidesPerView: 4,
    ...
    const allowedPages = [ "contact.html" ];
    if (allowedPages.some((page => currentPage.includes(page)))) window.addEventListener("load", (() => {
        setTimeout((() => {
            document.documentElement.classList.add("loaded");
        }), 300);
    }));
    const inputPhone = document.querySelector(".input-phone");
    const codeButton = document.querySelector(".form__code-button");
    const codeItem = document.querySelector(".form__code-item");
    const checks = document.querySelectorAll(".input-check");
    const buttonCheck = document.querySelector(".button-check");
    const checkboxFocus = document.querySelector(".checkbox__input");
    let isFirstClick = true;
    function checkConditions() {
        let allValid = true;
        checks.forEach((check => {
            if (check.value.trim() !== "12") allValid = false;
        }));
        if (allValid && checkboxFocus.checked) buttonCheck.classList.add("button-check--active"); else buttonCheck.classList.remove("button-check--active");
    }
    if (inputPhone) inputPhone.addEventListener("input", (() => {
        const phoneValue = inputPhone.value.trim();
        if (phoneValue.length >= 10) codeButton.classList.add("form__code-button--active"); else codeButton.classList.remove("form__code-button--active");
    }));
    if (codeButton) codeButton.addEventListener("click", (() => {
        if (codeButton.classList.contains("form__code-button--active")) if (isFirstClick) {
            codeItem.style.display = "block";
            isFirstClick = false;
        } else {
            let allValid = true;
            checks.forEach((check => {
                if (check.value.trim() !== "12") {
                    check.style.borderColor = "red";
                    allValid = false;
                } else check.style.borderColor = "";
            }));
            if (allValid) {
                alert("The message was sent successfully!");
                codeItem.style.display = "none";
                codeButton.classList.remove("form__code-button--active");
                checkConditions();
            }
        }
    }));
    if (checkboxFocus) checkboxFocus.addEventListener("change", checkConditions);
    checks.forEach((check => {
        check.addEventListener("input", checkConditions);
    }));
    document.addEventListener("DOMContentLoaded", (() => {
        const steps = document.querySelectorAll(".progress-bar__step");
        const lines = document.querySelectorAll(".progress-bar__line");
        const blockSteps = document.querySelectorAll(".step");
        if (steps && lines) steps.forEach(((step, index) => {
            step.addEventListener("click", (() => {
                const stepNumber = step.getAttribute("data-step");
                steps.forEach(((s, i) => {
                    if (i <= index) s.classList.add("progress-bar__step--active"); else s.classList.remove("progress-bar__step--active");
                    if (s.classList.contains("progress-bar__step--active")) s.style.setProperty("animation", "up 0s ease 0s forwards"); else s.style.setProperty("animation", "up-op 0s ease 0s forwards");
                }));
                lines.forEach(((line, i) => {
                    if (i < index) line.classList.add("progress-bar__line--active"); else line.classList.remove("progress-bar__line--active");
                }));
                blockSteps.forEach((block => {
                    if (block.getAttribute("data-step") === stepNumber) block.classList.add("step--active"); else block.classList.remove("step--active");
                }));
            }));
        }));
    }));
