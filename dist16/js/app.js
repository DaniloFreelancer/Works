(() => {
    ...
    setTimeout((() => {
        if (document.documentElement.classList.contains("loaded")) {
            function createSeamlessScroll({trackSelector, bodySelector, speed = 1.5, gap = 24}) {
                const track = document.querySelector(trackSelector);
                const bodyTrack = document.querySelector(bodySelector);
                if (!track || !bodyTrack) return;
                const duplicateContent = track.innerHTML;
                track.innerHTML += duplicateContent;
                let position = 0;
                const resetPoint = track.scrollWidth / 2 + gap;
                bodyTrack.addEventListener("mouseover", (() => currentSpeed = 0));
                bodyTrack.addEventListener("mouseout", (() => currentSpeed = speed));
            ...
            createSeamlessScroll({
                trackSelector: ".main__items",
                bodySelector: ".main__body",
                speed: 1.5,
                gap: window.innerWidth <= 400 ? 10 : 22
            });
            createSeamlessScroll({
                trackSelector: ".footer__items",
                bodySelector: ".footer__body",
                speed: 1.2,
                gap: window.innerWidth <= 400 ? 10 : 22
            });
        }
    }), 7e3);
    const allowedPages = [ "Magazine.html", "present.html", "presentPodcast.html", "presentAuthors.html", "podcast.html", "authors.html" ];
