(() => {
    ...
    function initSliders() {
        ...
        if (document.querySelector(".testimonial__slider")) new swiper_core_Swiper(".testimonial__slider", {
...
***************************************************************************** */
    ...
    const addQuantityHandlers = popupItems => {
        popupItems.querySelectorAll(".quantity__decrease").forEach((button => {
            button.addEventListener("click", (event => {
                event.stopPropagation();
                const id = button.closest(".popup__item").dataset.id;
                const product = cart.find((item => item.id === id));
                if (product && product.quantity > 1) product.quantity -= 1; else cart = cart.filter((item => item.id !== id));
                saveCartToLocalStorage();
                updatePopupContent(popupItems);
            }));
        }));
        popupItems.querySelectorAll(".quantity__increase").forEach((button => {
            button.addEventListener("click", (event => {
                event.stopPropagation();
                const id = button.closest(".popup__item").dataset.id;
                const product = cart.find((item => item.id === id));
    ...
    const updatePopupContent = popupItems => {
    ...
    const initCart = () => {
        const productButtons = document.querySelectorAll(".add-cart");
        const popup = document.getElementById("popup");
        const popupItems = popup.querySelector(".popup__items");
        ...
        productButtons.forEach((button => {
            button.addEventListener("click", (() => {
                ...
                if (existingProduct) existingProduct.quantity += 1; else cart.push({
                    id: productId,
                    name: productName,
                    price: productPrice,
                    image: productImage,
