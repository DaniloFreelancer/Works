(() => {
    ...
    function initSliders() {
        if (document.querySelector(".swiper")) {
            new swiper_core_Swiper(".item-body__slider", {
            ...
            new swiper_core_Swiper(".blog__slider", {
    ...
    let userData = JSON.parse(localStorage.getItem("popupData")) || {};
    function renderPopup() {
        const popupBody = popup.querySelector(".popup__body");
        popupBody.innerHTML = "";
        Object.entries(userData).forEach((([category, items]) => {
            const container = document.createElement("div");
            container.classList.add("popup__content-body");
            container.setAttribute("data-category", category);
            const title = document.createElement("h3");
            title.classList.add("popup__body-title");
            title.textContent = category;
            const itemsContainer = document.createElement("div");
            itemsContainer.classList.add("popup__body-items");
            let total = 0;
            items.forEach(((item, index) => {
                const itemElement = document.createElement("div");
                itemElement.classList.add("popup__body-item");
                const image = document.createElement("div");
                image.classList.add("popup__item-image");
                image.innerHTML = `<img src="${item.image}" alt="Item">`;
                const price = document.createElement("div");
                price.classList.add("popup__price");
                price.innerHTML = `Price: <span>${item.price}</span>$`;
                const removeBtn = document.createElement("button");
                removeBtn.classList.add("popup__remove");
            ...
            totalBlock.classList.add("popup__totalPrice");
            totalBlock.innerHTML = `Total price: <span>${total}$</span>`;
    ...
    function addToPopup(e) {
        const button = e.currentTarget;
        const slide = button.closest(".item-body__slide");
        const category = slide.getAttribute("data-category") || "Pillows";
        const imageSrc = slide.querySelector("img").getAttribute("src");
        const priceText = button.querySelector("span").textContent.replace("$", "");
        const item = {
            image: imageSrc,
            price: priceText
        };
        ...
        localStorage.setItem("popupData", JSON.stringify(userData));
        renderPopup();
    }
    function removeItem(category, index) {
        userData[category].splice(index, 1);
        if (userData[category].length === 0) delete userData[category];
        localStorage.setItem("popupData", JSON.stringify(userData));
        renderPopup();
    }
    const pillowPrices = document.querySelectorAll(".swiper-slide__price");
    pillowPrices.forEach((pillow => {
        pillow.addEventListener("click", addToPopup);
    }));
    renderPopup();
    window["FLS"] = false;
    menuInit();
